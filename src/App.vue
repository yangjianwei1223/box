    ...
    return {
      ...
      boxArr: [
        ...
        },
        {
          ...
          value: [
            { x: [20, 100], y: 60 },
            { x: [60, {type: 0, value: [{x: [30, 30], y: 30}, {x: [60], y: 30}]}], y: 60 }
          ]
        },
        {
          type: 0,
  ...
  methods: {
    changev(i) {
      ...
      if (!this.x1 && this.x1 != 0) {
        ...
        let islast =
            this.bigv.value[this.x].x[this.y].value[this.x1].x.length == this.y1 + 1 ? true : false; // 是否最后一个盒子
        if (this.bigv.value[this.x].x[this.y].value[this.x1].x.length == 1) {
            alert("当前不能再进行删除操作");
            return false;
        }
        if (isfirst) {
            this.bigv.value[this.x].x[this.y].value[this.x1].x[this.y1 + 1] += this.bigv.value[this.x].x[this.y].value[this.x1].x[this.y1];
            this.bigv.value[this.x].x[this.y].value[this.x1].x.shift();
        } else if (islast) {
            this.bigv.value[this.x].x[this.y].value[this.x1].x[this.y1 - 1] += this.bigv.value[this.x].x[this.y].value[this.x1].x[
            this.y1
            ];
            ...
            if (typeof vv.value[i].x[j] == 'number') {
                ...
                for (let m = 0; m < vv.value[i].x[j].value.length; m++) {
                    ...
                    for (let n = 0; n < vv.value[i].x[j].value[m].x.length; n++) {
                        vv.value[i].x[j].value[m].x[n] = vv.value[i].x[j].value[m].x[n] * 5; 
                    }
                }
            }
        }
